<template>
  <div class="set-red-light">
    <div class="switch-buttons">
      <el-button
        size="small"
        :type="openDrawRed ? 'primary' : ''"
        @click="switchopenDrawRed"
        >画红灯位置</el-button
      >
      ...
      <el-button
        size="small"
        :type="openDrawStop ? 'primary' : ''"
        @click="switchopenDrawStop"
        >画停车线</el-button
      >
      <el-button
        size="small"
    ...
    <div
      class="box"
      :style="{
        width: `${imgInitialWidth}px`,
        height: `${imgInitialHeight}px`,
      }"
    >
...
// this.offLine()
  ...
  methods: {
    ...
    realX(currentX) {
      ...
      return Number(
        ((proportion * 10 ** index * currentX) / 10 ** index).toFixed(2)
      );
    },
    scaleY(currentY) {
      // X轴计算，防止浮点数影响
    ...
    realY(currentY) {
      ...
      return Number(
        ((proportion * 10 ** index * currentY) / 10 ** index).toFixed(2)
      );
    },
    initDraw(isInitial) {
      if (isInitial) {
